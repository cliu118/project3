...
def output_word_info(word_date, date2name_list):
    ...
    # sort by friends descending, user name descending
    date2name_list = sorted(date2name_list,  key=lambda x:(x[1], x[0]), 
        reverse=True)

    # format: word_date1 \t friends1 followers1 \t date2 name2 friends2 + ...
    cnt = 0
    output = word_date
    last = ''
    ...
    def reduce(self):
        ...
        for line in self:
            ...
            if last_word_date != cur_word_date:
                ...
                date2name_list = [(user_name, friends)]
            else:
                # same word, update date info
                date2name_list.append((user_name, friends))    

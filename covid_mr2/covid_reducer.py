...
def output_word_info(word_date, date2name_list):
    ...
    # sort by by user name ascending
    date2name_list = sorted(date2name_list,  key=lambda x:(x[0]), 
        reverse=False)

    # format: word_date1 \t name1 desc1 \t date2 name2 desc2 + ...
    cnt = 0
    output = word_date
    last = ''
    ...
    def reduce(self):
        ...
        for line in self:
            ...
            if last_word_date != cur_word_date:
                ...
                date2name_list = [(user_name, desc)]
            else:
                # same word, update date info
                date2name_list.append((user_name, desc))    
